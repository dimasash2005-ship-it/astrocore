...
function NewChatModal({ onClose, onCreated }: {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [{ data: a }, { data: p }] = await Promise.all([
        sb.from("agents").select("id,name,description,provider_id,avatar_color").order("created_at"),
  ...
  function getProvider(id: string | null) {
  ...
  const selectedProvider = selectedAgent ? getProvider(selectedAgent.provider_id) : null
    ...
    <div onClick={e => { if (e.target === e.currentTarget) onClose() }}
      ...
      <div style={{
        ...
        <div style={{
          ...
          padding: "18px 20px 14px", borderBottom: "0.5px solid rgba(255,255,255,0.07)",
        }}>
          <div>
          ...
          <button onClick={onClose} style={{ width: 30, height: 30, borderRadius: 8, border: "none", background: "rgba(255,255,255,0.06)", cursor: "pointer", color: T.t4, lineHeight: 0 }}>
            <X size={14} />
          </button>
        </div>
        ...
        <div style={{ overflowY: "auto", padding: "14px 20px 20px", flex: 1 }}>
          {providers.length === 0 && (
            <div style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "11px 14px", borderRadius: 10, marginBottom: 14, background: "rgba(232,0,42,0.07)", border: "0.5px solid rgba(232,0,42,0.22)" }}>
              <AlertCircle size={14} style={{ color: T.red, flexShrink: 0, marginTop: 1 }} />
              <div>
                <div style={{ fontSize: 12.5, color: "#FF4D6A", fontWeight: 500, marginBottom: 4 }}>Немає API ключів</div>
                <div style={{ fontSize: 11.5, color: T.t4, lineHeight: 1.5 }}>Щоб агенти могли відповідати, потрібен API ключ.</div>
                <button onClick={() => { onClose(); router.push("/providers") }} style={{ marginTop: 8, fontSize: 11.5, color: T.red, background: "none", border: "none", cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 4 }}>
          ...
          {agents.length === 0 ? (
            <div style={{ padding: "32px 0", textAlign: "center" }}>
              <div style={{ width: 56, height: 56, borderRadius: 16, margin: "0 auto 14px", background: "rgba(232,0,42,0.08)", border: "0.5px solid rgba(232,0,42,0.18)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                <Bot size={24} style={{ color: T.red, opacity: 0.7 }} />
              </div>
              <div style={{ fontSize: 14, fontWeight: 600, color: T.t1, marginBottom: 6 }}>Немає агентів</div>
              <div style={{ fontSize: 12.5, color: T.t3, marginBottom: 18, lineHeight: 1.55 }}>Спочатку створіть AI агента з підключеним провайдером.</div>
              <button onClick={() => { onClose(); router.push("/agents") }} style={{ display: "inline-flex", alignItems: "center", gap: 7, padding: "9px 20px", borderRadius: 9, border: "none", background: T.red, color: "#fff", fontSize: 13, fontWeight: 500, cursor: "pointer" }}>
                <Plus size={14} /> Створити агента
              </button>
            </div>
          ) : (
            <>
              <div style={{ fontSize: 10, fontWeight: 600, color: T.t4, textTransform: "uppercase", letterSpacing: "0.09em", marginBottom: 10 }}>Оберіть агента</div>
                  ...
                  return (
                    <div key={agent.id}
                      ...
                      style={{
                        ...
                        background: active ? "linear-gradient(100deg,rgba(232,0,42,0.18) 0%,rgba(232,0,42,0.06) 100%)" : "rgba(255,255,255,0.03)",
                        border: active ? "1px solid rgba(232,0,42,0.38)" : "0.5px solid rgba(255,255,255,0.08)",
                    ...
                    >
                      <div style={{ width: 40, height: 40, borderRadius: 11, flexShrink: 0, background: agent.avatar_color ?? T.red, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, fontWeight: 700, color: "#fff", boxShadow: active ? `0 0 14px ${agent.avatar_color ?? T.red}50` : "none" }}>
                        {agent.name.charAt(0).toUpperCase()}
                      </div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 13.5, fontWeight: 500, color: active ? T.t1 : T.t2, marginBottom: 3 }}>{agent.name}</div>
                        {agent.description && <div style={{ fontSize: 11.5, color: T.t4, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", marginBottom: 5 }}>{agent.description}</div>}
                        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                          {prov ? (
                            <>
                              <span style={{ fontSize: 10.5, padding: "2px 7px", borderRadius: 5, background: active ? "rgba(232,0,42,0.12)" : "rgba(255,255,255,0.05)", border: `0.5px solid ${active ? "rgba(232,0,42,0.25)" : "rgba(255,255,255,0.08)"}`, color: active ? T.t2 : T.t4 }}>{prov.name}</span>
                              <span style={{ fontSize: 10.5, padding: "2px 7px", borderRadius: 5, background: "rgba(255,255,255,0.04)", border: "0.5px solid rgba(255,255,255,0.07)", color: T.t4 }}>{prov.model}</span>
                              <span style={{ fontSize: 10.5, padding: "2px 7px", borderRadius: 5, background: prov.is_active ? "rgba(34,197,94,0.09)" : "rgba(255,255,255,0.04)", border: `0.5px solid ${prov.is_active ? "rgba(34,197,94,0.22)" : "rgba(255,255,255,0.07)"}`, color: prov.is_active ? T.green : T.t4 }}>{prov.is_active ? "Активний" : "Вимкнено"}</span>
                            </>
                          ) : (
                            <span style={{ fontSize: 10.5, padding: "2px 7px", borderRadius: 5, background: "rgba(232,0,42,0.08)", border: "0.5px solid rgba(232,0,42,0.18)", color: "#FF4D6A", display: "flex", alignItems: "center", gap: 4 }}>
                              <AlertCircle size={9} /> Провайдер не знайдено
                            </span>
                          )}
                        </div>
                      </div>
                      <div style={{ width: 18, height: 18, borderRadius: "50%", flexShrink: 0, border: active ? "2px solid rgba(232,0,42,0.8)" : "1.5px solid rgba(255,255,255,0.15)", background: active ? "rgba(232,0,42,0.25)" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", transition: "all 140ms ease" }}>
        ...
        {agents.length > 0 && (
          <div style={{ padding: "12px 20px 16px", borderTop: "0.5px solid rgba(255,255,255,0.07)", display: "flex", gap: 10 }}>
            <button onClick={onClose} style={{ flex: 1, padding: "9px", borderRadius: 9, fontSize: 13, cursor: "pointer", background: "rgba(255,255,255,0.04)", border: "0.5px solid rgba(255,255,255,0.10)", color: T.t2 }}
            ...
            >
              ...
              {loading ? "Створюємо..." : canCreate ? `Почати чат з ${selectedAgent?.name}` : selected ? "Провайдер не підключено" : "Оберіть агента"}
            </button>
          </div>
        )}
      </div>
  ...
  return (
    ...
    >
      <div style={{
        ...
        {agent ? agent.name.charAt(0).toUpperCase() : <MessageSquare size={16} style={{ color: T.red, opacity: 0.7 }} />}
      </div>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 3 }}>
          ...
          {agent && (
            <span style={{ fontSize: 10, padding: "1px 6px", borderRadius: 4, flexShrink: 0, background: "rgba(255,255,255,0.05)", border: "0.5px solid rgba(255,255,255,0.08)", color: T.t4 }}>
              {agent.name}
            </span>
          )}
        </div>
        {session.last_message ? (
          <div style={{ fontSize: 11.5, color: T.t4, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
            <span style={{ color: session.last_role === "user" ? T.t3 : T.t4, fontWeight: session.last_role === "user" ? 500 : 400 }}>
              {session.last_role === "user" ? "Ви: " : "AI: "}
            </span>
            {cut(session.last_message, 80)}
          </div>
        ) : (
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
        ...
        <button className="del-btn" onClick={onDelete} style={{ opacity: 0, padding: 4, borderRadius: 6, border: "none", background: "none", cursor: "pointer", lineHeight: 0, transition: "opacity 140ms ease, color 140ms ease", color: T.t4 }}
...
function EmptyState({ onNew }: { onNew: () => void }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "80px 24px", textAlign: "center", width: "100%" }}>
      ...
      <button onClick={onNew} style={{ display: "flex", alignItems: "center", gap: 7, background: T.red, color: "#fff", border: "none", borderRadius: 10, padding: "10px 22px", fontSize: 13, fontWeight: 500, cursor: "pointer" }}
      ...
      <div style={{ marginTop: 18, fontSize: 10.5, color: "#3A3A5A", textTransform: "uppercase", letterSpacing: "0.10em" }}>Chat Layer · AI Conversations</div>
    </div>
  )
}
...
export default function ChatPage() {
  ...
  async function load() {
    const sb = getSupabase()
    const [{ data: sessionsData }, { data: agentsData }] = await Promise.all([
      sb.from("chat_sessions").select("*").order("updated_at", { ascending: false }),
      sb.from("agents").select("id,name,avatar_color"),
    ])
  ...
  function getAgent(agentId: string | null) { return agents.find(a => a.id === agentId) }
          ...
          <div style={{ position: "relative", zIndex: 1, display: "flex", alignItems: "flex-end", justifyContent: "space-between", flexWrap: "wrap", gap: 16 }}>
            <div>
              <div style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "rgba(232,0,42,0.08)", border: `0.5px solid ${T.bRed}`, borderRadius: 20, padding: "3px 10px", marginBottom: 14 }}>
                ...
                <span style={{ fontSize: 10, color: T.red, fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase" }}>Chat Layer · {sessions.length} сесій</span>
              </div>
              <h1 style={{ fontSize: 28, fontWeight: 600, color: T.t1, margin: 0, letterSpacing: "-0.02em" }}>Чати</h1>
              <p style={{ fontSize: 13, color: T.t3, marginTop: 6, marginBottom: 0 }}>AI Conversations · всі розмови з агентами в одному місці</p>
            </div>
          ...
          <div style={{ padding: "24px 48px 56px", maxWidth: 1100 }}>
            ...
            <div style={{ display: "flex", gap: 10, marginBottom: 22, flexWrap: "wrap" }}>
              {[
                ...
                { label: "Повідомлень",    value: sessions.reduce((s, c) => s + (c.message_count ?? 0), 0), icon: Zap        },
            ...
            {/* Search */}
            <div style={{ display: "flex", alignItems: "center", gap: 10, background: T.s1, border: `0.5px solid ${T.b1}`, borderRadius: 11, padding: "0 14px", marginBottom: 24, height: 42 }}>
              <Search size={15} style={{ color: T.t4, flexShrink: 0 }} />
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Пошук по чатах..."
                ...
                {groups.map(({ label, items }) => (
                  <div key={label}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
                      <span style={{ fontSize: 10, fontWeight: 600, color: T.t4, textTransform: "uppercase", letterSpacing: "0.10em" }}>{label}</span>
                    ...
                    <div style={{ display: "flex", flexDirection: "column" }}>
                      {items.map(session => (
                        <SessionCard
                          key={session.id}
                          session={session}
                          agent={getAgent(session.agent_id) as Agent | undefined}
